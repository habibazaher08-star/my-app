...
function handleUpdate(productId:string , count:number){
  ...
  {CartData?.numOfCartItems > 0 ?  <div className="flex mt-8 gap-8">
    ...
    <tbody>
      {CartData?.data.products.map((prod)=>{
        ...
        <td className="px-6 py-4">
          ...
          }}  className="font-medium text-fg-danger hover:underline">Remove</span>
        </td>
      </tr>
      })}
    </tbody>
  </table>
  <button onClick={()=>{RemoveCart()}} className=" mt-5 mb-5 w-full bg-green-600 text-white py-3 rounded-lg font-medium hover:bg-green-700 transition">
      Clear Cart
    </button>
</div>
</div>
  ...
  <div className="bg-white rounded-lg shadow-md p-4 sm:p-6 sticky top-4">
    ...
    <div className="space-y-3 mb-4">
      ...
      <div className="flex justify-between">
        ...
        <span className="font-medium">{CartData?.data.totalCartPrice} EGP</span>
      </div>
...
<svg viewBox="0 0 48 32" className="w-10 h-6">
 ...
 <svg viewBox="0 0 48 32" className="w-10 h-6">
  ...
  </div> : <div className="flex flex-col items-center justify-center text-center py-16">
  <Image
    src={cartImg}
    alt="cart"
    width={200}
